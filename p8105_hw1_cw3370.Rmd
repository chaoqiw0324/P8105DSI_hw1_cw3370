...
Dataset penguins has `r ncol(penguins)` variables, which are `r names(penguins)`. `r names(penguins)[c(1,2,7)]` are factor, `r names(penguins)[c(3,4)]` are number, `r names(penguins)[c(5,6,8)]` are integer. It is a tibble with `r nrow(penguins)` rows and `r ncol(penguins)` cols. The mean flipper length is `r mean(pull(penguins,var=5),na.rm = TRUE)`.
```{r}
